...
class Ignore(commands.Cog):
  ...
  async def command_add(self, ctx: commands.Context, command_name: str):
      ...
      if not command:
          embed = discord.Embed(title="<:olympus_cross:1227866668152393789> Error", description=f"`{command_name}` is not a valid command.", color=self.color)
  ...
  async def command_remove(self, ctx: commands.Context, command_name: str):
      ...
      async with aiosqlite.connect(self.db_path) as db:
          ...
          result = await cursor.fetchone()
          if not result:
              embed = discord.Embed(title="<:olympus_cross:1227866668152393789> Error", description=f"`{command_name}` is not in the ignore commands list.", color=self.color)
              await ctx.reply(embed=embed)
          else:
  ...
  async def channel_add(self, ctx: commands.Context, channel: discord.TextChannel):
    async with aiosqlite.connect(self.db_path) as db:
      ...
      if result:
        embed = discord.Embed(title="<:olympus_cross:1227866668152393789> Error", description=f"{channel.mention} is already in the ignore channels list.", color=self.color)
        await ctx.reply(embed=embed, mention_author=False)
      else:
  ...
  async def channel_remove(self, ctx: commands.Context, channel: discord.TextChannel):
    async with aiosqlite.connect(self.db_path) as db:
      ...
      if not result:
        embed = discord.Embed(title="<:olympus_cross:1227866668152393789> Error", description=f"{channel.mention} is not in the ignore channels list.", color=self.color)
        await ctx.reply(embed=embed)
      else:
  ...
  async def user_add(self, ctx: commands.Context, user: discord.User):
    async with aiosqlite.connect(self.db_path) as db:
      ...
      if result:
        embed = discord.Embed(title="<:olympus_cross:1227866668152393789> Error", description=f"{user.mention} is already in the ignore users list.", color=self.color)
        await ctx.reply(embed=embed, mention_author=False)
      else:
  ...
  async def user_remove(self, ctx: commands.Context, user: discord.User):
    async with aiosqlite.connect(self.db_path) as db:
      ...
      if not result:
        embed = discord.Embed(title="<:olympus_cross:1227866668152393789> Error", description=f"{user.mention} is not in the ignore users list.", color=self.color)
        await ctx.reply(embed=embed)
      else:
  ...
  async def bypass_add(self, ctx: commands.Context, user: discord.User):
    async with aiosqlite.connect(self.db_path) as db:
      ...
      if result:
        embed = discord.Embed(title="<:olympus_cross:1227866668152393789> Error", description=f"{user.mention} is already in the bypass users list.", color=self.color)
        await ctx.reply(embed=embed, mention_author=False)
      else:
